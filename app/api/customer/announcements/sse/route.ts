...
export async function GET(req: NextRequest) {
  ...
  const consumer = createConsumer(`sse-announcements-${Date.now()}-${Math.random()}`);
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      try {
        ...
        await consumer.subscribe({ topic: "store-announcements" });
        await consumer.run({
          eachMessage: async ({ message }) => {
            if (!message.value) return;
            const data = JSON.parse(message.value.toString());
            // Only stream to clients connected to the target store
        ...
        console.error("SSE Kafka error:", err);
      }
